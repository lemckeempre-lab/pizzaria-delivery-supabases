...
const PRODUTOS_PADRAO = [
  {nome:"Margherita Classica",  desc:"Molho de tomate, mozzarella, manjericao fresco",         preco:42.90,cat:"Pizzas",    emoji:"🍕",badge:"Classica", ativo:true},
  {nome:"Pepperoni Suprema",    desc:"Molho especial, mozzarella, pepperoni importado",         preco:52.90,cat:"Pizzas",    emoji:"🔥",badge:"Top Venda",ativo:true},
  {nome:"Quatro Queijos",       desc:"Mozzarella, parmesao, gorgonzola, provolone",             preco:55.90,cat:"Pizzas",    emoji:"🧀",badge:null,       ativo:true},
  {nome:"Frango com Catupiry",  desc:"Molho branco, frango desfiado, catupiry, milho",          preco:49.90,cat:"Pizzas",    emoji:"🍗",badge:"Favorita", ativo:true},
  {nome:"Calabresa Artesanal",  desc:"Molho artesanal, calabresa defumada, cebola caramelizada",preco:47.90,cat:"Pizzas",    emoji:"🌶️",badge:null,      ativo:true},
  {nome:"Combo Familia",        desc:"2 Pizzas grandes + 2 Refrigerantes 2L + Sobremesa",       preco:139.90,cat:"Combos",  emoji:"👨‍👩‍👧‍👦",badge:"Oferta",  ativo:true},
  {nome:"Combo Casal",          desc:"1 Pizza grande + 1 Pizza pequena + 2 Refrigerantes",      preco:89.90,cat:"Combos",   emoji:"💑",badge:"Popular",  ativo:true},
  {nome:"Coca-Cola 2L",         desc:"Refrigerante gelado 2 litros",                            preco:14.90,cat:"Bebidas",  emoji:"🥤",badge:null,       ativo:true},
  {nome:"Suco Natural 500ml",   desc:"Laranja, maracuja ou limao espremido na hora",            preco:12.90,cat:"Bebidas",  emoji:"🍊",badge:"Fresco",   ativo:true},
  {nome:"Tiramisu da Casa",     desc:"Receita italiana com mascarpone e cafe expresso",         preco:22.90,cat:"Sobremesas",emoji:"🍰",badge:"Especial", ativo:true},
];
...
const getSlugFromURL = () => { const path = window.location.pathname.replace(/^\/+/,"").split("/")[0]; return path || null; };
...
function GlobalCSS({ p="#C41E3A", d="#8b0f24" }) {
  return <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .fade-up{animation:fadeUp .35s ease both} .pop-in{animation:popIn .3s cubic-bezier(.34,1.56,.64,1) both}
    ...
    .btn-p:hover{background:var(--d);transform:translateY(-1px)} .btn-p:disabled{opacity:.6;cursor:not-allowed}
...
function Spinner({ color="#C41E3A", size=36 }) {
  return <div style={{width:size,height:size,border:`3px solid rgba(0,0,0,.1)`,borderTop:`3px solid ${color}`,borderRadius:"50%",animation:"spin .9s linear infinite",margin:"0 auto"}}/>;
}
...
function LoadingScreen({ dark=false }) {
  return (
    <div style={{minHeight:"100vh",background:dark?"#0f0f1a":"#FFF8F0",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center"}}><div style={{fontSize:52,marginBottom:20}}>🍕</div><Spinner color={dark?"#C77DFF":"#C41E3A"}/></div>
    </div>
  );
}
...
// PIZZARIA APP
// ══════════════════════════════════════════════════════════
function PizzariaApp({ loja: lojaInit, slugId }) {
  ...
  const marca = { nome:loja.nome, logo:loja.logo||"🍕", tagline:loja.tagline||"Pizzas artesanais quentinhas na sua porta", pix:loja.pix||"", tema:TEMAS.find(t=>t.nome===loja.tema)||TEMAS[0] };
  ...
  useEffect(() => { loadData(); }, [slugId]);

  const loadData = async () => {
    setLoading(true);
    const [{ data: prods }, { data: peds }] = await Promise.all([
      supabase.from("produtos").select("*").eq("loja_slug", slugId).order("id"),
      supabase.from("pedidos").select("*").eq("loja_slug", slugId).order("criado_em", { ascending: false }),
    ]);
    setProdutos(prods && prods.length > 0 ? prods.map(p => ({...p, desc: p.descricao})) : PRODUTOS_PADRAO.map((p,i)=>({...p,id:i+1})));
    setPedidos((peds||[]).map(p => ({...p, num: p.num_end})));
    setLoading(false);
  };

  const saveProduto = async (form, editProd) => {
    const row = { loja_slug:slugId, nome:form.nome, descricao:form.desc, preco:parseFloat(form.preco)||0, cat:form.cat, emoji:form.emoji, badge:form.badge||null, ativo:true };
    if (editProd) { await supabase.from("produtos").update(row).eq("id", editProd.id); }
    else { await supabase.from("produtos").insert([row]); }
    await loadData();
  };
  ...
  const confirmPedido = async (ped) => {
    await supabase.from("pedidos").insert([{ loja_slug:slugId, numero:ped.numero, nome:ped.nome, tel:ped.tel, rua:ped.rua, num_end:ped.num, bairro:ped.bairro, cidade:ped.cidade||"", comp:ped.comp||"", pgto:ped.pgto, itens:ped.itens, total:ped.total, status:"Recebido", horario:ped.horario, data_str:ped.data }]);
    setCart([]);
    await loadData();
  };

  const updatePedidoStatus = async (numero, status) => {
    await supabase.from("pedidos").update({status}).eq("numero", numero).eq("loja_slug", slugId);
    setPedidos(ps=>ps.map(p=>p.numero===numero?{...p,status}:p));
  };

  const saveMarca = async (nm) => {
    await supabase.from("lojas").update({ nome:nm.nome, logo:nm.logo, tagline:nm.tagline, pix:nm.pix, tema:nm.tema?.nome||"Brasa" }).eq("slug", slugId);
  ...
  const filtered = produtos.filter(p => {
    ...
    if (search) { const q=search.toLowerCase(); return p.nome.toLowerCase().includes(q)||(p.desc||"").toLowerCase().includes(q); }
    return true;
  });
    ...
    <>
      ...
      <nav style={{position:"sticky",top:0,zIndex:100,background:tema.primary,boxShadow:"0 2px 20px rgba(0,0,0,.2)"}}>
        <div style={{maxWidth:960,margin:"0 auto",padding:"0 20px",height:62,display:"flex",alignItems:"center",gap:12}}>
          <button onClick={()=>setShowLogin(true)} style={{background:"none",display:"flex",alignItems:"center",gap:8,flexShrink:0,padding:"4px 0"}}>
            <span style={{fontSize:26}}>{marca.logo}</span>
            <span style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:700,fontSize:20,color:"#FFF8F0"}}>{marca.nome}</span>
          </button>
          <div style={{flex:1,position:"relative"}}>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar pizzas..." style={{width:"100%",padding:"8px 14px 8px 36px",borderRadius:99,border:"none",background:"rgba(255,255,255,.18)",color:"#fff",fontSize:14}}/>
      ...
      <div style={{background:"linear-gradient(135deg,"+tema.primary+","+tema.dark+")",padding:"52px 20px 44px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{maxWidth:640,margin:"0 auto",position:"relative"}}>
          <div style={{fontSize:56,marginBottom:14}}>{marca.logo}</div>
          <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:700,fontSize:"clamp(30px,6vw,50px)",color:"#FFF8F0",lineHeight:1.15,marginBottom:12}}>{marca.nome}</h1>
          <p style={{color:"rgba(255,248,240,.75)",fontSize:16,lineHeight:1.6,marginBottom:24,maxWidth:440,margin:"0 auto 24px"}}>{marca.tagline}</p>
            ...
            {filtered.map((p,i)=>(
              <div key={p.id} className="fade-up card-hov" style={{animationDelay:i*25+"ms",background:"#fff",borderRadius:"var(--radius)",border:"1px solid var(--border)",overflow:"hidden",display:"flex",flexDirection:"column"}}>
                ...
                <div style={{padding:"14px 16px 16px",flex:1,display:"flex",flexDirection:"column",gap:7}}>
                  ...
                  <p style={{color:"var(--muted)",fontSize:12,lineHeight:1.5,flex:1}}>{p.desc||p.descricao}</p>
      ...
      {showCheckout&&<CheckoutModal cart={cart} onClose={()=>setShowCheckout(false)} onConfirm={confirmPedido} marca={marca}/>}
      {showLogin&&<LoginModal marca={marca} adminPass={loja.admin_pass} onLogin={()=>{setLoggedIn(true);setShowLogin(false);setShowAdmin(true);}} onClose={()=>setShowLogin(false)}/>}
      {showAdmin&&<PizzariaAdminPanel onClose={()=>setShowAdmin(false)} pedidos={pedidos} onUpdateStatus={updatePedidoStatus} produtos={produtos} onSaveProduto={saveProduto} onDeleteProduto={deleteProduto} onToggleProduto={toggleProduto} marca={marca} onSaveMarca={saveMarca}/>}
...
function CheckoutModal({ cart, onClose, onConfirm, marca }) {
  ...
  const confirm = async () => {
    setSaving(true);
    await onConfirm({numero:pedNum,...form,itens:cart,total,horario:nowStr(),data:dateStr(),status:"Recebido"});
    setSaving(false); setStep(3);
  };
  const tema = marca.tema||TEMAS[0];
  return (
    <>
            ...
            <div style={{textAlign:"center",padding:"16px 0"}}>
              ...
              {form.pgto==="pix"&&marca.pix&&(
                <div style={{background:"#f0fdf4",borderRadius:12,padding:"14px",border:"1px solid #bbf7d0",marginBottom:16}}>
                  ...
                  <p style={{fontSize:15,fontWeight:700}}>{marca.pix}</p>
                </div>
              )}
...
function LoginModal({ marca, adminPass, onLogin, onClose, isMaster=false }) {
      ...
      <div className="modal-wrap">
        <div className="modal" style={{maxWidth:360}}>
          <div style={{textAlign:"center",marginBottom:22}}>
            <span style={{fontSize:36}}>{isMaster?"🔐":marca?.logo||"🍕"}</span>
            ...
            {!isMaster&&<p style={{color:"var(--muted)",fontSize:13,marginTop:3}}>{marca?.nome}</p>}
          </div>
...
function PizzariaAdminPanel({ onClose, pedidos, onUpdateStatus, produtos, onSaveProduto, onDeleteProduto, onToggleProduto, marca, onSaveMarca }) {
  const [tab, setTab] = useState("pedidos");
  const [newProd, setNewProd] = useState(false);
  const [editProd, setEditProd] = useState(null);
  const [form, setForm] = useState({nome:"",desc:"",preco:"",cat:"Pizzas",emoji:"🍕",badge:""});
  const [mForm, setMForm] = useState({nome:marca.nome,tagline:marca.tagline,pix:marca.pix,logo:marca.logo,tema:marca.tema?.nome||"Brasa"});
  ...
  const saveProd = async () => {
    setSaving(true); await onSaveProduto(form, editProd); setSaving(false);
    setNewProd(false); setEditProd(null); setForm({nome:"",desc:"",preco:"",cat:"Pizzas",emoji:"🍕",badge:""});
  };
  const saveMarca = async () => {
    setSaving(true);
    const tema = TEMAS.find(t=>t.nome===mForm.tema)||TEMAS[0];
    await onSaveMarca({...marca,...mForm,tema});
      ...
      <div className="drawer" style={{width:"min(560px,100vw)"}}>
        <div style={{padding:"16px 22px",background:"#1a1008",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:19,fontWeight:700,color:"#FFF8F0"}}>Painel Admin</h2><p style={{color:"rgba(255,248,240,.45)",fontSize:11,marginTop:1}}>{marca.nome}</p></div>
          </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"18px 22px"}}>
          {tab==="pedidos"&&(
            <div style={{display:"grid",gap:12}}>
              {pedidos.length===0?(<div style={{textAlign:"center",padding:"50px 0",color:"var(--muted)"}}><div style={{fontSize:44,marginBottom:10}}>📦</div><p>Nenhum pedido ainda</p></div>)
              :[...pedidos].map(ped=>{
                const sc = STATUS_COLOR[ped.status]||STATUS_COLOR["Recebido"];
                return (
                  <div key={ped.numero} style={{background:"#fff",borderRadius:14,padding:"16px",border:"1px solid var(--border)"}}>
          ...
          {tab==="cardapio"&&(
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                ...
                <button className="btn-p" style={{padding:"8px 16px",fontSize:13}} onClick={()=>{setEditProd(null);setNewProd(true);setForm({nome:"",desc:"",preco:"",cat:"Pizzas",emoji:"🍕",badge:""});}}>+ Novo</button>
              </div>
              {newProd&&(
                <div style={{background:"#fff",borderRadius:14,padding:"18px",border:"2px solid var(--p)",marginBottom:14}}>
                  ...
                  <div style={{display:"grid",gap:9}}>
                    ...
                    <div><label style={{fontSize:10,fontWeight:600,color:"var(--muted)",display:"block",marginBottom:3}}>DESCRICAO</label><input className="inp" value={form.desc} onChange={e=>up("desc",e.target.value)}/></div>
                ...
                {produtos.map(p=>(
                  <div key={p.id} style={{background:"#fff",borderRadius:12,padding:"12px 14px",border:"1px solid var(--border)",display:"flex",alignItems:"center",gap:10,opacity:p.ativo?1:.5}}>
                    ...
                    <div style={{display:"flex",gap:5}}>
                      <button onClick={()=>{setEditProd(p);setNewProd(true);setForm({nome:p.nome,desc:p.desc||p.descricao||"",preco:String(p.preco),cat:p.cat,emoji:p.emoji,badge:p.badge||""});}} style={{background:"#f5f0eb",borderRadius:8,padding:"5px 8px",fontSize:13}}>✏️</button>
          ...
          {tab==="marca"&&(
            <div style={{display:"grid",gap:16}}>
              ...
              <div><label style={{fontSize:10,fontWeight:600,color:"var(--muted)",display:"block",marginBottom:5}}>NOME DA PIZZARIA</label><input className="inp" value={mForm.nome} onChange={e=>upM("nome",e.target.value)}/></div>
              ...
              <div>
                <label style={{fontSize:10,fontWeight:600,color:"var(--muted)",display:"block",marginBottom:8}}>COR DO TEMA</label>
...
function MasterPanel() {
  ...
  const saveLoja = async () => {
    ...
    if (editId) {
      await supabase.from("lojas").update({nome:form.nome,dono:form.dono,tel:form.tel,email:form.email,pix:form.pix,admin_pass:form.adminPass||"Pizza@Delivery2026!",tema:form.tema,logo:form.logo,tagline:form.tagline}).eq("id", editId);
    } else {
      const { data: nova } = await supabase.from("lojas").insert([{slug:s,nome:form.nome,dono:form.dono,tel:form.tel,email:form.email,pix:form.pix,admin_pass:form.adminPass||"Pizza@Delivery2026!",tema:form.tema,logo:form.logo,tagline:form.tagline,ativa:true,plano:"Basico"}]).select().single();
      if (nova) {
        await supabase.from("produtos").insert(PRODUTOS_PADRAO.map(p=>({loja_slug:s,nome:p.nome,descricao:p.desc,preco:p.preco,cat:p.cat,emoji:p.emoji,badge:p.badge||null,ativo:true})));
      }
    }
    await loadLojas();
    setSaving(false);
    ...
    <div style={{minHeight:"100vh",background:"#0f0f1a",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{display:"flex",borderBottom:"1px solid rgba(255,255,255,.07)",padding:"0 28px"}}>
        {[["lojas","🍕 Pizzarias"],["add","➕ Nova pizzaria"]].map(([id,label])=>(
          <button key={id} onClick={()=>{setTab(id);if(id==="add"){setEditId(null);setForm({nome:"",dono:"",tel:"",email:"",pix:"",adminPass:"",tema:"Brasa",logo:"🍕",tagline:""});}}} style={{padding:"14px 20px",borderBottom:"2px solid "+(tab===id?"#C77DFF":"transparent"),background:"transparent",color:tab===id?"#C77DFF":"rgba(255,255,255,.4)",fontWeight:tab===id?700:400,fontSize:13,cursor:"pointer",transition:"all .2s",fontFamily:"inherit"}}>{label}</button>
        ))}
      </div>
      <div style={{padding:"24px 28px",maxWidth:1000,margin:"0 auto"}}>
        {tab==="lojas"&&(
          <div>
            {loading?(<div style={{textAlign:"center",padding:"80px 0"}}><div style={{width:36,height:36,border:"3px solid rgba(199,125,255,.2)",borderTop:"3px solid #C77DFF",borderRadius:"50%",animation:"spin .9s linear infinite",margin:"0 auto"}}/></div>)
            :lojas.length===0?(
                  ...
                  return (
                    <div key={l.id} className="fade-up" style={{animationDelay:i*40+"ms",background:"rgba(255,255,255,.04)",border:"1px solid "+(l.ativa?"rgba(255,255,255,.1)":"rgba(255,0,0,.15)"),borderRadius:16,padding:"18px 22px",display:"flex",alignItems:"center",gap:16}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <p style={{color:"rgba(255,255,255,.4)",fontSize:12}}>{l.dono&&l.dono+" · "}{l.tel&&l.tel+" · "}Desde {new Date(l.criado_em).toLocaleDateString("pt-BR")}</p>
        ...
        {tab==="add"&&(
          <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.1)",borderRadius:20,padding:"28px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <div><label style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.4)",display:"block",marginBottom:5}}>SENHA ADMIN</label><input className="inp-d" value={form.adminPass} onChange={e=>up("adminPass",e.target.value)} placeholder="Senha segura"/></div>
              <div>
            ...
            {form.nome&&(
              <div style={{marginTop:18,background:"rgba(255,255,255,.04)",borderRadius:12,padding:"12px 16px",display:"flex",alignItems:"center",gap:10}}>
                <span style={{fontSize:14,color:"rgba(255,255,255,.4)"}}>Link gerado:</span>
            ...
            <div style={{display:"flex",gap:10,marginTop:20}}>
              ...
              <button onClick={saveLoja} disabled={!form.nome||saving} style={{background:form.nome?"linear-gradient(135deg,#6b2fa0,#C41E3A)":"rgba(255,255,255,.1)",color:form.nome?"#fff":"rgba(255,255,255,.3)",borderRadius:99,padding:"11px 28px",fontSize:14,fontWeight:600,cursor:form.nome?"pointer":"not-allowed",fontFamily:"inherit",opacity:saving?.7:1}}>
                {saving?"Salvando...":(editId?"Salvar":"Cadastrar pizzaria →")}
              </button>
            </div>
          </div>
        )}
...
export default function App() {
    ...
    if (!masterLogado) return (
      <>
        ...
        <div style={{minHeight:"100vh",background:"#0f0f1a",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <LoginModal isMaster marca={null} onLogin={()=>setMasterLogado(true)} onClose={()=>window.history.back()}/>
        </div>
      </>
    );
    return <MasterPanel/>;
  ...
  if (slugAtual) {
    ...
    return <PizzariaApp loja={loja} slugId={slugAtual}/>;
  }
